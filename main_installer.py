...
if os.geteuid() != 0:
    ...
    try:
        # Need QApplication before QMessageBox if not yet created
        from PyQt5.QtWidgets import QApplication, QMessageBox
        _dummy_app = QApplication.instance() or QApplication(sys.argv)
        QMessageBox.critical(None,"Root Required","This installer requires root privileges.\nPlease run using 'sudo'.")
    except Exception: pass # Fallback to console print already done
    sys.exit(1)

# --- Attempt Archinstall Import ---
# (Run this after privilege check)
try:
    import archinstall
    # Example: Import specific items if needed and API is known
    # from archinstall import profile, Installer, models, disk
    ...
    try:
        from PyQt5.QtWidgets import QApplication, QMessageBox # Already imported above, but safe
        _dummy_app = QApplication.instance() or QApplication(sys.argv)
        QMessageBox.critical(None,"Dependency Error","Required 'archinstall' library not found.\nPlease install it and restart.")
...
SUMMARY_STEP_INTRO_TEXT = "Review settings. <b style='color:yellow;'>Install button modifies disk!</b>" # Reverted
    ...
    try:
        ...
        for path,d in raw_devs.items():
            ...
            if dtype=='disk' and isinstance(size,int) and size>(1*1024**3):
                devs.append({'name':path,'size':size,'type':dtype,'label':getattr(d,'label',None)})
...
def get_profiles():
    try: p_dict=archinstall.profile.list_profiles(); p_list=[{"name":n,"description":getattr(p,'desc','No description')} for n,p in p_dict.items() if not n.startswith('_')]; return p_list if p_list else [{"name":"Minimal","description":"Basic system."}]
    except Exception as e: print(f"WARN get_profiles: {e}"); return [{"name":"Minimal","description":"Basic system."}]

# --- Qt Log Handler ---
class QtLogHandler(logging.Handler):
    def __init__(self, log_signal_emitter): super().__init__(); self.sig=log_signal_emitter; self.setFormatter(logging.Formatter('%(levelname)s:%(message)s'))
...
class InstallationThread(QThread):
    ...
    def setup_logging(self):
        log = logging.getLogger('archinstall'); log.setLevel(logging.INFO)
        ...
        self.log_signal.emit("Log capture setup.") # Correct indentation

    def map_categories_to_packages(self, cats):
        mapping={"Programming":["git","code"],"Gaming":["steam"],"Office":["libreoffice-fresh"],"Graphics":["gimp"]}
        pkgs = set()
        for c in cats: pkgs.update(mapping.get(c,[])) # Use update for sets
        return list(pkgs)

    def run(self):
        self.setup_logging(); self.log_signal.emit("Starting install thread..."); self.progress_signal.emit(0,"Preparing...")
        try:
            ai_conf = self.config.copy() # Work on a copy
            
            # --- Translate GUI config to archinstall config ---
            # Locale/Keyboard - Assuming top-level keys now
            ai_conf['keyboard_layout'] = ai_conf.pop('keyboard_layout', DEFAULT_CONFIG['keyboard_layout'])
            ai_conf['locale'] = ai_conf.pop('locale', DEFAULT_CONFIG['locale'])

            # Mirror Region
            ai_conf['mirror_region'] = ai_conf.get('mirror_region', DEFAULT_CONFIG['mirror_region']) # Assumes code stored

            # Disk Config - Needs verification based on archinstall API
            target_disk = ai_conf.get('harddrives', [])[0] if ai_conf.get('harddrives') else None
            if not target_disk: raise ValueError("Target disk not specified.")
            filesystem = ai_conf.pop('disk_filesystem', DEFAULT_CONFIG['disk_config']['filesystem']) # Get FS from old key if exists
            ai_conf['disk_config'] = {
                '!layout': { target_disk: {'wipe': True, 'filesystem': {'format': filesystem}}}
            } # Recreate disk_config

            # Encryption
            if ai_conf.get('disk_encrypt'):
                ai_conf['disk_encryption'] = [{'device': target_disk, 'password': ai_conf.pop('disk_encrypt_password'), 'type': 'luks'}]
            else:
                 ai_conf.pop('disk_encryption', None); ai_conf.pop('disk_encrypt_password', None)

            # User/Root Passwords (already using !users, !root-password keys)
            users = []
            if ai_conf.get('username'): # Check if username was actually entered
                users.append({
                    'username': ai_conf.pop('username'),
                    'password': ai_conf.pop('user_password'),
                    'sudo': ai_conf.pop('user_sudo', True) # Assume true if key exists? Default carefully.
                })
            else: # Ensure keys are removed if no user
                 ai_conf.pop('username', None); ai_conf.pop('user_password', None); ai_conf.pop('user_sudo', None)
            ai_conf['!users'] = users
            ...
            additional_pkgs = ai_conf.get('packages', [])
            ai_conf['packages'] = list(set(category_pkgs + additional_pkgs))

            # Branding/Custom Commands (assuming archinstall uses this key)
            branding_name = ai_conf.get('_os_brand_name', 'Mai Bloom OS')
            ai_conf['custom_commands'] = [
                f'sed -i \'s/^PRETTY_NAME=.*/PRETTY_NAME="{branding_name}"/\' /etc/os-release',
                # Add more commands here to run inside chroot if archinstall supports this key
            ]

            # --- Clean up internal GUI keys ---
            keys_to_remove = ['_app_categories', '_os_brand_name', 'disk_filesystem', 'disk_encrypt']
            for key in keys_to_remove: ai_conf.pop(key, None)

            # Log final config (excluding sensitive info)
            ...
            # archinstall.Installer(mountpoint='/mnt', config=ai_conf).install()
            # Or: archinstall.run_as_guided_installation(ai_conf) # If such a function exists
...
class ConfigSectionWidget(QGroupBox):
    # (Unchanged)
    def __init__(self, title, config_ref, main_window_ref): super().__init__(title); self.config=config_ref; self.main_window=main_window_ref; self.content_layout=QVBoxLayout(self); self.content_layout.setSpacing(10); self.content_layout.setContentsMargins(10,20,10,10)
    def load_ui_from_config(self): pass
    def save_config_from_ui(self): return True

# --- Concrete Config Section Widgets ---
# (LocaleKeyboardSection, DiskSection, UserSection, ProfileAppsSection - unchanged)
...
class DiskSection(ConfigSectionWidget):
    ...
    def load_ui_from_config(self): self.populate_disks(); self.fs.setCurrentText(self.config.get('disk_filesystem',DEFAULT_CONFIG['disk_config']['filesystem'])); enc=self.config.get('disk_encryption')is not None; self.enc_cb.setChecked(enc); self.enc_pw.setEnabled(enc); self.enc_pw.clear()
    def save_config_from_ui(self):
        if self.dc.currentIndex()<0 or not self.dc.currentData(): QMessageBox.warning(self,"","Select disk."); return False
        disk=self.dc.currentData(); self.config['harddrives']=[disk]; fs=self.fs.currentText(); self.config['disk_filesystem'] = fs # Store FS for potential direct use or mapping
        # Storing disk_config structure is done in the thread now based on these simpler keys
        self.config['disk_encrypt'] = self.encrypt_cb.isChecked() # Store boolean
        if self.config['disk_encrypt']: pw=self.enc_pw.text();
                                     if len(pw)<8: QMessageBox.warning(self,"","Encrypt pwd >= 8 chars."); return False
                                     self.config['disk_encrypt_password'] = pw # Store separately
        else: self.config.pop('disk_encrypt_password', None)
        return True
class UserSection(ConfigSectionWidget):
    ...
    def save_config_from_ui(self): hn=self.hn.text().strip();rp=self.rp.text();usr=self.un.text().strip();up=self.up.text();
                             ...
                             self.config['hostname']=hn; self.config['!root-password']=rp; # Use direct archinstall key
                             if usr:
                                 if len(up)<8 or up!=self.upc.text(): QMessageBox.warning(self,"","User pwd err."); return False
                                 # Store directly in the structure archinstall expects
                                 self.config['!users'] = [{'username':usr,'password':up,'sudo':self.sudo.isChecked()}]
                                 # Keep temporary copies if needed for display, but they are not passed to thread
                                 self.config['username'] = usr # Keep for summary logic maybe?
                                 self.config['user_sudo'] = self.sudo.isChecked()
                             else: self.config['!users']=[]; self.config.pop('username',None); self.config.pop('user_sudo',None) # Clear user list and temp keys
                             self.config.pop('user_password', None) # Don't store user password directly if in !users
                             self.config.pop('root_password', None) # Don't store root password directly if in !root-password
                             return True
class ProfileAppsSection(ConfigSectionWidget):
     def __init__(self, config, main_ref): super().__init__("🖥️ Profile & Applications", config, main_ref); layout=QVBoxLayout(); layout.setSpacing(10); layout.addWidget(QLabel("Base Profile:")); self.prof_cb=QComboBox(); self.profs=get_profiles(); [self.prof_cb.addItem(p['description'],p['name'])for p in self.profs]; layout.addWidget(self.prof_cb); layout.addWidget(QLabel("App Categories (Optional):")); self.cats={"Programming":"💻","Gaming":"🎮","Office":"📄","Graphics":"🎨"}; self.cbs={}; grid=QGridLayout();r,c=0,0;
...
class MaiBloomOSInstallerWindow(QMainWindow): # (init_ui, populate_steps, navigation, etc. remain the same)
    def __init__(self): super().__init__(); self.config_data=DEFAULT_CONFIG.copy(); self.installation_thread=None; self.init_ui() # Removed step tracking
    def init_ui(self): # Simplified UI setup from previous KISS version
        self.setWindowTitle(f"{APP_NAME}"); self.setMinimumSize(1000,750);
        ...
        # Add Configuration Sections directly
    ...
    def save_and_validate_all_sections(self): self.appendToLog("Saving config...","DEBUG");
                                        for sec in self.config_sections:
                                            if not sec.save_config_from_ui(): self.appendToLog(f"Validation failed: {sec.title()}","ERROR"); QMessageBox.warning(self,"Error",f"Check '{sec.title()}'."); return False
    ...
    def confirm_and_start_installation(self):
        ...
        summary="Review Config:\n" + "\n".join([f"- {k}: {'<set>' if 'pass' in k else v}" for k,v in self.config_data.items() if k not in ['disk_encryption','!users']]) # Basic summary
