...
class CalamaresAutomatorApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"Fully Automated Calamares Installer (Arch Linux) - Storage Helper Edition")
    ...
    def _define_automation_stages(self):
        if not self.calamares_download_url or not self.temp_dir_path:
             ...
             return False
        self.automation_stages = [
            {"name": "Clean Pacman Cache", "function": self.run_step_1b_clean_pacman_cache, "progress_target": 10}, # Adjusted
            {"name": "Install Dependencies", "function": self.run_step_2_install_dependencies, "progress_target": 20}, # Adjusted
            {"name": "Download Calamares", "function": self.run_step_3_download_calamares, "progress_target": 35}, # Adjusted
            {"name": "Extract Calamares", "function": self.run_step_4_extract_calamares, "progress_target": 50}, # Adjusted
            {"name": "Configure Calamares (CMake)", "function": self.run_step_5_cmake_calamares, "progress_target": 65}, # Adjusted
            {"name": "Compile Calamares (Make)", "function": self.run_step_6_make_calamares, "progress_target": 90}, # Adjusted
            {"name": "Run Calamares", "function": self.run_step_7_run_calamares, "progress_target": 100}, # Adjusted
        ]
        return True
    ...
    def run_next_automation_stage(self):
        if self.current_stage_index < len(self.automation_stages):
            ...
            prev_progress = self.automation_stages[self.current_stage_index-1]['progress_target'] if self.current_stage_index > 0 else 5 # Start from 5 after fetch
            self.set_progress_bar_value(prev_progress + 1)
    ...
    def start_step_1_fetch_release_info(self):
        ...
        try:
            ...
            for asset in assets:
                if asset.get("name", "").endswith(".tar.gz") and GH_REPO_NAME in asset.get("name", ""):
                    if self.calamares_version.lstrip('v') in asset.get("name"):
                        ...
                        break
            if not source_tarball_asset_url:
            ...
            self.setWindowTitle(f"Automated Calamares {self.calamares_version} Installer - Storage Helper")
            ...
            self.set_progress_bar_value(5) # Progress after info fetch
            ...
            if self._define_automation_stages():
                ...
                pass # _define_automation_stages handles critical error
    ...
    # --- NEW STAGE ---
    def run_step_1b_clean_pacman_cache(self):
        self.log_message("Cleaning pacman cache (sudo pacman -Scc --noconfirm)...")
        command = ["pacman", "-Scc", "--noconfirm"]
        self.run_qprocess_command(command, "Pacman Cache Clean")
    # --- END NEW STAGE ---
    ...
    def run_step_3_download_calamares(self):
        ...
        initial_progress = self.automation_stages[self.current_stage_index-1]['progress_target']
        ...
        try:
            response = requests.get(self.calamares_download_url, stream=True, timeout=120)
            response.raise_for_status()
            ...
            self.set_progress_bar_value(initial_progress + target_progress_span)
    ...
    def run_step_7_run_calamares(self):
        ...
        if not os.path.exists(calamares_executable):
            calamares_executable = os.path.join(self.build_path, "calamares")
            if not os.path.exists(calamares_executable):
    ...
    def run_qprocess_command(self, command_list, process_name, working_directory=None):
        ...
        if not self.qprocess.waitForStarted(7000):
    ...
    def handle_qprocess_finished(self, exit_code, exit_status, process_name):
        QTimer.singleShot(50, self.handle_stdout)
        QTimer.singleShot(50, self.handle_stderr)
    ...
    def closeEvent(self, event):
        ...
        if self.qprocess and self.qprocess.state() == QProcess.Running:
            ...
            if not self.qprocess.waitForFinished(3000):
                self.log_message("Process did not terminate gracefully, killing.")
                self.qprocess.kill()
                self.qprocess.waitForFinished(1000)
...
if __name__ == '__main__':
    if os.geteuid() != 0:
        ...
        try:
            ...
            print(f"Could not show GUI error message: {e}")
        sys.exit(1)
