...
class InstallerWindow(QWidget):
    ...
    def onProceed(self):
        ...
        time.sleep(2)
        subprocess.Popen(["konsole", "-e", "bash", "-c", "archinstall && arch-chroot && sudo pacman -Syu git --no-confirm && cd omnipkg-app/ && chmod +x config.sh && sudo bash config.sh; exec bash"])
        self.close()
